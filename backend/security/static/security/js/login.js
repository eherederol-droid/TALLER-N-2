(function() {
    ...
    function validate() {
        ...
        if (!email) { showError('El correo electr\u00f3nico es requerido');
            els.email.focus(); return false; }
        if (!EMAIL_REGEX.test(email)) { showError('Ingrese un correo electr\u00f3nico v\u00e1lido');
            els.email.focus(); return false; }
        if (!password) { showError('La contrase\u00f1a es requerida');
            els.password.focus(); return false; }
        if (password.length < 4) { showError('La contrase\u00f1a debe tener al menos 4 caracteres');
            els.password.focus(); return false; }
        return true;
    }
